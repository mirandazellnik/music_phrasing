...
cols = ["Note", "Time", "Len_P", "Len_M", "D_A", "Exact", "Exact_L", "Exact_H", "Motion", "Len_Ratio"] + [f"W" + dist for dist in dists] + [f"B" + dist for dist in dists] + [f"A" + dist for dist in dists] + ["Velo", "Macro", "Micro"]
seq_total = {} if saving else {cname:[] for cname in cols}
...
for count, row in METADATA.iterrows():
    ...
    for n in range(len(new_starts)):
        ...
        for pos, j in enumerate(new_starts[n:max(n-100, 0):-1]):
            if j < new_starts[n]:
    ...
    lists = lists[:-1] + [down_while, exact, exact_l, exact_h, motion, length_ratio] + within + behind_only + ahead_only + [lists[-1]]
    ...
    if saving:
        ...
        seq_total[row["midi_performance"]] = seq
    else:
        for i, v in enumerate(lists):
            seq_total[cols[i]] += v
...
if saving:
    with open(os.path.join(PROCESSED_PATH, "total_data.json"), "w") as outfile:
        json.dump(seq_total, outfile)
else:
